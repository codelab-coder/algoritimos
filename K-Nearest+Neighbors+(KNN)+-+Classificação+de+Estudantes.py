...
# base na proximidade de amostras semelhantes
...
# 2. rotulos: a lista de rótulos correspondentes que classificam
...
# Função para exibir o menu e permitir entrada do usuário
def classificar_estudante():
    ...
    try:
        ...
        # Verifica se os valores inseridos são negativos, o que não seria válido
        ...
        # Exibe o resultado da classificação para o usuário, acessando o
    ...
    # strings que não podem ser convertidas para números
        ...
        # Exibe uma mensagem pedindo ao usuário que insira valores
...
def main():
    ...
    # Um loop infinito que continua executando até que o usuário decida sair
    while True:

        # Chama a função 'classificar_estudante' para permitir ao
        # usuário classificar um aluno.
        # Esta função solicita a entrada do usuário para a nota e
        # número de faltas do aluno, realiza a classificação
        # com o modelo KNN, e exibe o resultado.
        classificar_estudante()

        # Solicita ao usuário a decisão de continuar ou não
        # classificando outros alunos.
        # A função input() captura a entrada do usuário como uma string,
        # que é convertida para letras minúsculas com .lower()
        continuar = input("Deseja classificar outro aluno? (s/n): ").lower()

        # Verifica a resposta do usuário para determinar se o
        # loop deve continuar ou não
        # Se o usuário digitar qualquer coisa diferente de 's', o
        # loop é interrompido
        if continuar != 's':
